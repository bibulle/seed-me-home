...
describe('UserService', () => {
  ...
  beforeAll(async () => {
    ...
    jest.spyOn(jwtHelper, 'decodeToken').mockImplementation(token => {
      ...
      if (token === 'not-expired') {
        return { firstName: 'not', lastName: 'expired', googleUser: 'foo bar' };
      } else {
  ...
  it('Authentication should be tested every 3 second', done => {
    ...
    expect(user).toEqual({ firstName: 'not', lastName: 'expired', googleUser: 'foo bar' });
  ...
  it('Check login process (to google)', async done => {
    ...
    const req = httpMock.expectOne(environment.serverUrl + 'authentication/google/login?code=google-token');
