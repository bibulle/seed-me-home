...
} from './rtorrent-torrent-item.component';
import {
  MAT_DIALOG_DATA,
  MatDialog,
  MatDialogModule,
...
describe('RtorrentTorrentItemComponent', () => {
  ...
  it('should display items value', () => {
    ...
    expect(
      fixture.debugElement.query(By.css('.title span')).nativeElement
        .textContent
    ).toBe(torrent1.name);
    ...
    ).toBe((torrent1.size / 1024 / 1024 / 1024).toFixed(2) + ' GB');

    expect(fixture.debugElement.queryAll(By.css('.down')).length).toBe(1);
    expect(
      fixture.debugElement.query(By.css('.down')).nativeElement.textContent
    ).toBe(
      'cloud_download' +
        (torrent1.down_rate / 1024 / 1024).toFixed(2) +
        ' MB/s '
    );

    expect(fixture.debugElement.queryAll(By.css('.up')).length).toBe(1);
    expect(
      fixture.debugElement.query(By.css('.up')).nativeElement.textContent
    ).toBe('cloud_upload' + torrent1.up_rate + ' B/s ');

    expect(fixture.debugElement.queryAll(By.css('.ratio')).length).toBe(1);
    expect(
      fixture.debugElement.query(By.css('.ratio')).nativeElement.textContent
    ).toBe('compare_arrows' + torrent1.ratio.toFixed(2) + ' ');
    ...
    ).toBe(moment.utc(torrent1.addtime * 1000).format('lll'));
  });
  ...
  it('icon before name should be the right one', () => {
    ...
    expect(
      fixture.debugElement.query(By.css('.title mat-icon')).nativeElement
        .textContent
    ).toBe('cloud_download');
    expect(
      fixture.debugElement.queryAll(By.css('.progress mat-progress-bar'))[0]
        .attributes['ng-reflect-value']
    ).toBe('25');
    expect(
      fixture.debugElement.queryAll(By.css('.progress mat-progress-bar'))[1]
        .attributes['ng-reflect-value']
    ).toBe('0');
    // actions menu
    expect(
    ...
    let menuContent = fixture.debugElement.queryAll(
    ...
    ).toBe('pause[this is a fake translation of seed.pause]');
    expect(
      menuContent.queryAll(By.css('button'))[1].nativeElement.textContent
    ).toBe('delete_forever[this is a fake translation of seed.erase]');
    expect(
      menuContent.queryAll(By.css('button'))[2].nativeElement.textContent
    ).toBe('save_alt[this is a fake translation of seed.save_here]');
    // pause the torrent
    ...
    expect(
      fixture.debugElement.queryAll(By.css('.progress mat-progress-bar')).length
    ).toBe(2);
    expect(
      fixture.debugElement.queryAll(By.css('.progress mat-progress-bar'))[0]
        .attributes['ng-reflect-value']
    ...
    expect(
      menuContent.queryAll(By.css('button'))[0].nativeElement.textContent
    ).toBe('play_arrow[this is a fake translation of seed.start]');
    expect(
      menuContent.queryAll(By.css('button'))[1].nativeElement.textContent
    ).toBe('delete_forever[this is a fake translation of seed.erase]');
    expect(
    ...
    expect(
      fixture.debugElement.query(By.css('.title mat-icon')).attributes['color']
    ).toBe('warn');
    expect(
      fixture.debugElement.queryAll(By.css('.progress mat-progress-bar'))[0]
        .attributes['ng-reflect-value']
    ...
    expect(
      menuContent.queryAll(By.css('button'))[2].nativeElement.textContent
    ).toBe('block[this is a fake translation of seed.not_save_here]');
    // switch "should save" of the torrent
    menuContent.queryAll(By.css('button'))[2].nativeElement.click();
  ...
  it('ratio should be colored', () => {
    ...
    fixture.detectChanges();
    expect(
      fixture.debugElement.query(By.css('.ratio')).classes['very-bad']
    ).toBeTruthy();
    expect(
      fixture.debugElement.query(By.css('.ratio')).classes['good']
    ).toBeFalsy();
    expect(
      fixture.debugElement.query(By.css('.ratio')).classes['very-good']
    ).toBeFalsy();
  ...
  it('up and down rate should be highlighted', () => {
    ...
    fixture.detectChanges();
    expect(
      fixture.debugElement.query(By.css('.up')).classes['highlight']
    ).toBeFalsy();
    expect(
      fixture.debugElement.query(By.css('.down')).classes['highlight']
    ).toBeFalsy();
  ...
  it('sort item should be correct', () => {
    ...
    expect(fixture.debugElement.queryAll(By.css('.header .size')).length).toBe(
      1
    );
    expect(fixture.debugElement.queryAll(By.css('.header .down')).length).toBe(
      1
    );
    expect(fixture.debugElement.queryAll(By.css('.header .up')).length).toBe(1);
    expect(fixture.debugElement.queryAll(By.css('.header .ratio')).length).toBe(
      1
    );
    expect(fixture.debugElement.queryAll(By.css('.header .date')).length).toBe(
      1
    );
  ...
  it('click on header should toggle the sort', () => {
    ...
    fixture.debugElement
      .query(By.css('.header .up'))
      .triggerEventHandler('click', null);
    expect(selected).toBe('up');

    fixture.debugElement
      .query(By.css('.header .down'))
      .triggerEventHandler('click', null);
    expect(selected).toBe('down');

    fixture.debugElement
      .query(By.css('.header .ratio'))
      .triggerEventHandler('click', null);
